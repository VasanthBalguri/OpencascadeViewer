...
opencascade::handle<TDocStd_Document> createOCAFDoc(opencascade::handle<TDocStd_Application> &app)
{
    ...
    BRepPrimAPI_MakeWedge wedgebuilder(10.0,10.0,10.0,1.0);
    wedgebuilder.Build();
    TopoDS_Shape wedge =  wedgebuilder.Shape();

    TDF_Label s01 = doc->Main().NewChild();
    TNaming_Builder builder(s01);
    builder.Generated(wedge);
...
void subGeomHistory(opencascade::handle<BRepTools_History> &aHistory,TDF_Label &topLabel)
{
  int label = 1;

  for (; anExp.More(); anExp.Next()) {
      const TopoDS_Shape& anOldSub = anExp.Current();

        TDF_Label subFeature = topLabel.FindChild(label);
      // B. Record Sub-Shape Evolution (Faces, Edges, Vertices)
      // We iterate through all sub-shapes of the original object
      TNaming_Builder subBuilder(subFeature);

      TopExp_Explorer anExp(myBody, TopAbs_FACE);
      // Check for Modifications
      const TopTools_ListOfShape& aModified = aHistory->Modified(anOldSub);
      if (!aModified.IsEmpty()) {
          subBuilder.Modify(anOldSub, aModified.First());
      }

      // Check for Deletions
      if (aHistory->IsRemoved(anOldSub)) {
          subBuilder.Delete(anOldSub);
      }

      // Check for Generations (e.g., an Edge generating a Face in Extrude)
      const TopTools_ListOfShape& aGenerated = aHistory->Generated(anOldSub);
      if (!aGenerated.IsEmpty()) {
          subBuilder.Generated(anOldSub, aGenerated.First());
      }
  }
}
opencascade::handle<TDocStd_Document> createOCAFBottle(opencascade::handle<TDocStd_Application> &app,const Standard_Real myWidth, const Standard_Real myHeight,const Standard_Real myThickness)
{
  ...
  {
    doc->NewCommand();
    TDF_Label bottleFeature = ftreeLabel.FindChild(label);
  // Profile : Define Support Points
  gp_Pnt aPnt1(-myWidth / 2., 0, 0);
  gp_Pnt aPnt2(-myWidth / 2., -myThickness / 4., 0);
  gp_Pnt aPnt3(0, -myThickness / 2., 0);
  ...
  myBody = BRepAlgoAPI_Fuse(myBody, myNeck);
    ...
    TNaming_Builder bottleBuilder(bottleFeature);
  bottleBuilder.Generated(myBody);
  Handle(BRepTools_History) aHistory = bottleBuilder.History();
  doc->CommitCommand();
  label++;
  doc->NewCommand();
  TDF_Label bottleFeature2 = ftreeLabel.FindChild(label);
  // Body : Create a Hollowed Solid
  TopoDS_Face   faceToRemove;
  Standard_Real zMax = -1;
  ...
  aBuilder.Add (aRes, myThreading);
      ...
      TNaming_Builder bottleBuilder2(bottleFeature2);
  bottleBuilder2.Modify(myBody,aRes);
  doc->CommitCommand();
